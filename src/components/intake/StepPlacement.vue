...
<template>
  <div class="space-y-6">
    ...
    <div v-if="!store.answers.placement.skipped" class="space-y-5 lg:grid lg:grid-cols-2 lg:gap-4 lg:space-y-0">
      <!-- Horizontal push -->
      <fieldset class="rounded-xl border border-rule p-4">
        ...
        <label v-else-if="!branch.canFullPushup" class="mt-2 block text-sm">
          ...
          <input v-model.number="local.kneeReps" type="number" inputmode="numeric" class="mt-1 min-h-11 w-full rounded-xl border border-rule px-3" />
        </label>
        <label v-else class="mt-2 block text-sm">
          How many full push-ups can you do with good form before your hips sag?
          <input v-model.number="local.fullReps" type="number" inputmode="numeric" class="mt-1 min-h-11 w-full rounded-xl border border-rule px-3" />
        </label>
      </fieldset>
      ...
      <label class="block rounded-xl border border-rule p-4 text-sm">
        ...
        <input v-model.number="local.squatReps" type="number" inputmode="numeric" class="mt-2 min-h-11 w-full rounded-xl border border-rule px-3" />
      </label>
