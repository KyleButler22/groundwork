...
- [ ] **Grocery list generation** (`docs/mealgen.md` §8) — derive a `grocery_lists`/`grocery_items` pair from a generated `MealPlan`'s entries. Needs the unit/gram resolution logic `scripts/lib/ingredientIndex.mjs` already has on the authoring side, ported client-side (deliberately not pulled into `src/generators/meal/library.ts` yet — see that file's own note). The "fresh-cook entries only, leftovers contribute nothing" rule (§8) is now straightforward given `MealPlanEntry.servings`'s corrected semantics: sum a fresh entry's own servings plus every entry whose `leftoverOfId` points at it.
- [ ] Wire `MealsView.vue`/`GroceryView.vue` (currently placeholders) to call `generateMealPlan()` → Dexie, mirroring how `IntakeView.vue` already wires the workout generator.
- [ ] Fix `swapOneMeal`'s known limitation above (propagate a swap to its leftover).
