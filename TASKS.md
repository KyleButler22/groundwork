...
- [ ] **Meal plan generator** (`src/generators/meal/`) — implement `docs/mealgen.md` now that the corpus exists in seedable form. Mirror the workout generator's shape: pure TS modules (filter/allocate/score/assemble/repair), fixtures for fast unit tests, an integration spec against the real 200-recipe corpus. Needs food-domain types added to `src/types/domain.ts` and a Dexie `version(2)` bump in `db.ts`, plus extending `devContentSeed.ts` to seed the food-reference and recipe tables the same way it already seeds the movement library.
