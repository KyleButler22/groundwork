...
**Movement library (`0003_movement_library.sql`).** The real IP of the app. Modelled as `(pattern_id, level)` for the generator to walk, plus an explicit `progression_edges` table for branches — a single `next_exercise_id` column can't express a fork (e.g. full push-up → either archer or pseudo-planche track). `metric_type` (`reps` / `time_seconds` / `distance_m`) exists because isometrics (planks, hangs, L-sits) and holds don't have reps — assume reps everywhere and roughly a third of the library breaks. `distance_min_m`/`distance_max_m` cover handstand-walk-style exercises (added after the original draft shipped `distance_m` as an allowed `metric_type` with no columns to back it — a real bug, fixed before the seed data needed it). `cues` is a one-line reminder; `how_to` (all 60, added in `0011`) is the labeled-step version the exercise page shows — newline-separated `Label: detail` lines ending with a `Common mistake:` line, enforced by `parseMovementLibrarySeed`'s `assertSeedShape`. It ships as `update` statements, byte-identical in `001_movement_library.sql` (for the bundled parser) and migration `0011` (for the live DB, since `db reset` would wipe real user data); a later change to `how_to` ships as a new migration. No semicolons or `--` in the text — conservative hygiene (the `update`-statement form is parsed quote-aware, but the characters broke the positional-column design this replaced).
