...
- **Which slots to plan is an explicit per-slot choice, not a count** (changed 2026-08-29, in response to direct user feedback: "let me decide breakfast/lunch/dinner in any combination — I mostly just want dinner ideas"). The original design collapsed this to a single `mealsPerDay` number plus a fixed highest-share-first priority order (dinner, then lunch, then breakfast, then snack) — which can express "just dinner" (count 1) but can never express "breakfast and lunch, no dinner", since dinner always outranks them both. `UserTargets.activeMealSlots: MealSlot[]` (4 booleans in Postgres — `wants_breakfast`/`wants_lunch`/`wants_dinner`/`wants_snack`, see `0002_identity.sql`) replaces it; `grid.ts`'s `planActiveSlots` now just normalizes the caller's exact selection into canonical (highest-share-first) order rather than deriving a subset from a count. `meal_plan_entries`'s `unique (meal_plan_id, serve_on, slot)` still only has 4 slot values, so a duplicate/unrecognised entry is silently deduped rather than erroring — moot in practice since TypeScript already makes both cases unreachable from a real caller. An empty selection (which intake itself refuses to submit) falls back to dinner alone with a warning, as a defensive backstop only.
...
7 days × however many of the 4 slots the user asked to plan (`activeMealSlots`) — but not every slot needs its own recipe.
...
| Swap propagating to a leftover | **Resolved (2026-08-29)**: `swapOneMeal` now excludes the leftover from `lockedEntries` too and passes `dinnerDayPlanOverride` (`reconstructDinnerDayPlanFromEntries`) so assemble.ts's ordinary leftover-copy logic picks up the new recipe — no special-casing needed. Fixing this also surfaced and fixed an independent bug: with nearly every dinner day locked during a swap, a fresh random topology drawn from the swap's own derived seed could coincidentally reassign the UNLOCKED target day itself as a leftover of some unrelated locked day. |
